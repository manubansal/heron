 ...
 */
public class HomogeneousPacking implements IPacking {
  private static final Logger LOG = Logger.getLogger(HomogeneousPacking.class.getName());
  ...
  public PackingPlan pack() {
    System.out.println("HomogeneousPacking pack() called");

    // Get the instances' round-robin allocation
    Map<Integer, List<String>> trevorDummyAllocation = getTrevorDummyAllocation();

    // Get the ram map for every instance
    Map<Integer, Map<String, Long>> instancesRamMap =
        getInstancesRamMapInContainer(trevorDummyAllocation);

    long containerDiskInBytes = getContainerDiskHint(trevorDummyAllocation);
    double containerCpu = getContainerCpuHint(trevorDummyAllocation);

    // Construct the PackingPlan
    Set<PackingPlan.ContainerPlan> containerPlans = new HashSet<>();
    long topologyRam = 0;
    for (Map.Entry<Integer, List<String>> entry : trevorDummyAllocation.entrySet()) {
   ...
   */
  private Map<Integer, List<String>> getTrevorDummyAllocation() {
